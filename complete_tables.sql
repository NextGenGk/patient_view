...
create index IF not exists idx_doctors_clinic_id on public.doctors using btree (clinic_id) TABLESPACE pg_default;
...
create index IF not exists idx_transactions_razorpay_payment on public.finance_transactions using btree (razorpay_payment_id) TABLESPACE pg_default;
...
-- ignored_orders
create table public.ignored_orders (
  id uuid not null default extensions.uuid_generate_v4 (),
  order_id uuid not null,
  delivery_boy_id uuid not null,
  ignored_at timestamp with time zone null default now(),
  ...
  constraint ignored_orders_delivery_boy_id_fkey foreign KEY (delivery_boy_id) references delivery_agents (id) on delete CASCADE,
  constraint ignored_orders_order_id_fkey foreign KEY (order_id) references orders (id) on delete CASCADE
) TABLESPACE pg_default;
...
create index IF not exists idx_adherence_pid_date_time on public.medication_adherence using btree (pid, scheduled_date, scheduled_time) TABLESPACE pg_default
where
  (is_taken = false);
...
create index IF not exists medicines_name_idx on public.medicines using btree (name) TABLESPACE pg_default;

create index IF not exists medicines_category_idx on public.medicines using btree (category) TABLESPACE pg_default;

-- order_delivery_assignments
create table public.order_delivery_assignments (
  id uuid not null default extensions.uuid_generate_v4 (),
  order_id uuid not null,
  delivery_boy_id uuid not null,
  ...
  constraint order_delivery_assignments_delivery_boy_id_fkey foreign KEY (delivery_boy_id) references delivery_agents (id) on delete CASCADE,
  constraint order_delivery_assignments_order_id_fkey foreign KEY (order_id) references orders (id) on delete CASCADE
) TABLESPACE pg_default;
...
after
update OF sent_to_patient on prescriptions for EACH row when (
  old.sent_to_patient is distinct from new.sent_to_patient
  and new.sent_to_patient = true
)
